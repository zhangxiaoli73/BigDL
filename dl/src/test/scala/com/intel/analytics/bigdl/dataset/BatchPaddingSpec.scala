...
class BatchPaddingSpec extends FlatSpec with Matchers with BeforeAndAfter {
  "SampleToBatchPaddingLM " should "be good when batchsize = 1" in {
    ...
    val trainSet2 = DataSet.array(trainData, true)
      .transform(LabeledSentenceToSample(dictionaryLength))
  ...
  "SampleToBatchPaddingLM " should "be good for language model when batchsize > 1" in {
  ...
  "SampleToBatchPadding " should "be good for language model when batchsize > 1" in {
    val batchSize = 3
    val dictionaryLength = 5
    ...
    val storage1 = input2.storage().array() ++ input1.storage().array() ++
