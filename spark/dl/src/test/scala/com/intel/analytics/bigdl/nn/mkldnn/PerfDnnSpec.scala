...
class PerfDnnSpec extends FlatSpec with Matchers {
  ...
  "AlexNet 4-models 1-omp-thread with local optimizer" should "work correctly" in {
    ...
    println(s"AlexNet 4-models 1-omp-thread with local optimizer throughput = " +
      s"${16 * 50 / ((end - start).toDouble / 1e9)}")
  }
  ...
  "AlexNet 1-model 4-omp-threads with single thread pool" should "work correctly" in {
    System.setProperty("bigdl.localMode", "true")
    System.setProperty("bigdl.coreNumber", "1")
    System.setProperty("bigdl.mklNumThreads", s"$core")
    Engine.init
    ...
    val computing = new ThreadPool(1)
    computing.setMKLThread(4) // don't forget set mkl threads

    val time = manyTimes {
      computing.invokeAndWait(
        (0 until 1).map(i =>
          () => {
            model.forward(input)
            model.backward(input, gradOutput)
    ...
    println(taken / iters)
    println("AlexNet 1-model 4-omp-threads with single thread pool " +
      s"throughput = ${16 * iters / taken * 1000}")
  }
