...
object MTLabeledCaffeImgToBatch {
  def apply[A: ClassTag](width: Int, height: Int, batchSize: Int,
                         transformer: Transformer[A, LabeledBGRImage])
  : MTLabeledCaffeImgToBatch[A] = {
    new MTLabeledCaffeImgToBatch[A](
      width, height, batchSize, transformer)
  }
}

/**
 * Multi-thread convert caffe images into Mini-Batch.
 *
 ...
 * @tparam A
 */
class MTLabeledCaffeImgToBatch[A: ClassTag] private[bigdl](width: Int, height: Int,
   totalBatchSize: Int, transformer: Transformer[A, LabeledBGRImage])
      ...
      override def next(): MiniBatch[Float] = {
        ...
        val batch = Engine.default.invokeAndWait((0 until parallelism).map(tid => () => {
          ...
          while (iterators(tid).hasNext && {
            ...
            img.copyCaffeImg(featureData, position * frameLength * 3)
