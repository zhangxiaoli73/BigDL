...
class FPN2MLPFeatureExtractor(in_channels: Int, resolution: Int,
  scales: Array[Float], sampling_ratio: Int, representation_size: Int)
  (implicit ev: TensorNumeric[Float]) extends AbstractModule[Table, Tensor[Float], Float] {

  val pooler = new Pooler(resolution, scales, sampling_ratio)
  val input_size = in_channels * math.pow(resolution, 2)
  val fc6 = Linear[Float](input_size.toInt, representation_size, withBias = true)
  val fc7 = Linear[Float](representation_size, representation_size, withBias = true)
