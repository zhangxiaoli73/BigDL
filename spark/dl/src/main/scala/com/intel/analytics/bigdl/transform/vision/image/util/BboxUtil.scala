...
object BboxUtil {
  ...
  def decodeSingleBbox(i: Int, priorBox: Tensor[Float], priorVariance: Tensor[Float],
  ...
  def decodeSignalBoxNew(encodeBox: Tensor[Float], bbox: Tensor[Float],
  ...
  def decode(encodeBox: Tensor[Float], bbox: Tensor[Float],
             weight: Tensor[Float], decodeBox: Tensor[Float]): Unit = {
    ...
    var i = 1
    while (i <= numBboxes) {
      decodeSignalBoxNew(encodeBox.select(1, i), bbox.select(1, i), weight, decodeBox.select(1, i))
      i += 1
    }
  }
