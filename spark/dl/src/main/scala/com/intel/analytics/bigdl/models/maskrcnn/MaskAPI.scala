...
abstract class SegmentationMask {
...
case class COCORLE(counts: Array[Int], height: Int, width: Int) extends SegmentationMask {
  override def toRLETensor(): Tensor[Float] = {
    val t = Tensor[Float](counts.length)
    val arr = t.storage().array()
    for (i <- 0 to (counts.length - 1)) {
      arr(i) = counts(i).toFloat
    }
    t
  }

  def get(idx: Int): Long = {
    MaskAPI.uint2long(counts(idx).toInt)
  }
}
object MaskAPI {
  ...
  def mergeRLEs(R: Array[COCORLE], intersect: Boolean = false): COCORLE = {
    val n = R.length
    if (n == 1) return R(0)
    val h = R(0).height
    val w = R(0).width
  ...
  def polyToSingleRLE(poly: COCOPoly, height: Int, width: Int): COCORLE = {
  ...
  // from colume
  def binaryToRLE(binaryMask: Tensor[Float]): COCORLE = {
    ...
    COCORLE(countsBuffer.toArray, height = h, width = w)
  }
}
