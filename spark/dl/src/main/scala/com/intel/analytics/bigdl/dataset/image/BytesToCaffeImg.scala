...
object BytesToCaffeImg {
  def apply(normalize: Float = 255f, resizeW : Int = -1, resizeH : Int = -1): BytesToCaffeImg =
    new BytesToCaffeImg(normalize, resizeW, resizeH)
}
 ...
 * @param normalize
 */
class BytesToCaffeImg(normalize: Float, resizeW : Int = -1, resizeH : Int = -1)
  ...
  override def apply(prev: Iterator[ByteRecord]): Iterator[LabeledBGRImage] = {
    prev.map(rawData => {
      buffer.copySrc(getImgData(rawData, resizeW, resizeH), normalize).setLabel(rawData.label)
    })
  }
