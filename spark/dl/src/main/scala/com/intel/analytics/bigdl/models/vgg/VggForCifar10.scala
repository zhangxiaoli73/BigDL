...
import com.intel.analytics.bigdl.nn.mkldnn.{MemoryBackward, MemoryFoward, MemoryReOrder}
...
object VggForCifar10 {
  def apply(classNum: Int, hasDropout: Boolean = true): Module[Float] = {
    ...
    : Sequential[Float] = {
      ...
      vggBnDo.add(nn.SpatialBatchNormalization(nOutPutPlane, 1e-3))
...
//      vggBnDo.add(MemoryFoward(5))
//      vggBnDo.add(nn.SpatialBatchNormalization(nOutPutPlane, 1e-3).setName("dnn"))
//      vggBnDo.add(MemoryBackward(5))
    ...
    if (hasDropout) classifier.add(mkldnn.DropoutDnn(0.5))
    classifier.add(mkldnn.Linear(512, classNum))
    classifier.add(LogSoftMax())
    vggBnDo.add(classifier)
