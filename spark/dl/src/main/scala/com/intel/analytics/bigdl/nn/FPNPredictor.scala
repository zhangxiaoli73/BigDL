...
class FPNPredictor(num_class: Int, in_channels: Int, num_bbox_reg_classes: Int)
  (implicit ev: TensorNumeric[Float]) extends AbstractModule[Tensor[Float], Table, Float] {
  val cls_score = Linear[Float](in_channels, num_class)
  // todo: check with torch
  cls_score.weight.apply1(_ => RNG.normal(0, 0.01).toFloat)
  cls_score.bias.fill(0.0f)

  val bbox_pred = Linear[Float](in_channels, num_bbox_reg_classes * 4)
