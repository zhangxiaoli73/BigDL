...
object CaffeImgToSample {
  def apply(toRGB: Boolean = true): CaffeImgToSample = {
    new CaffeImgToSample(toRGB)
  }
}
 ...
 */
class CaffeImgToSample(toRGB: Boolean = true) extends Transformer[LabeledBGRImage, Sample[Float]] {
  ...
  override def apply(prev: Iterator[LabeledBGRImage]): Iterator[Sample[Float]] = {
    prev.map(img => {
      ...
      img.copyCaffeImg(featureBuffer.storage().array(), 0)
