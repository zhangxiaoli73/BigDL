...
class LSTMNew[T : ClassTag] (
    ...
    if (p != 0) {
      i2g = Sequential()
        .add(ConcatTable()
          ...
          .add(Dropout(p)))
        .add(ParallelTable()
          .add(LinearNew(inputSize, hiddenSize,
      ...
      h2g = Sequential()
        .add(ConcatTable()
          ...
          .add(Dropout(p)))
        .add(ParallelTable()
          .add(LinearNew(hiddenSize, hiddenSize,
      ...
      i2g = LinearNew(inputSize, 4 * hiddenSize,
        wRegularizer = wRegularizer, bRegularizer = bRegularizer)
      h2g = LinearNew(hiddenSize, 4 * hiddenSize,
        withBias = false, wRegularizer = uRegularizer)
    }
  ...
  override def canEqual(other: Any): Boolean = other.isInstanceOf[LSTMNew[T]]
  ...
  override def toString: String = s"LSTMNew($inputSize, $hiddenSize, $p)"
}
...
object LSTMNew {
  def apply[@specialized(Float, Double) T: ClassTag](
    ...
    (implicit ev: TensorNumeric[T]): LSTMNew[T] = {
    new LSTMNew[T](inputSize, hiddenSize, p, wRegularizer, uRegularizer, bRegularizer)
  }
}
