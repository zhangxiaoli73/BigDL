  ...
  def loadWeights(module: Module[Float], identity: String): Unit = {
    ...
    module match {
      case bn: SpatialBatchNormalization =>
        ...
        val weight = DnnTools.getTensor(s"Fwrd_${bn.getName}.Wght.3", Array(channel), identity)
        val bias = DnnTools.getTensor(s"Fwrd_${bn.getName}.Wght.4", Array(channel), identity)
      ...
      case _ =>
        for (j <- params.indices) {
          val w = DnnTools.getTensor(s"Fwrd_$name.Wght.$j", params(j).size(), identity)
          module match {
  ...
  def compareGradients(module: Module[Float], epsilon: Float, identity: String): Boolean = {
    ...
    module match {
      case bn: SpatialBatchNormalization =>
        ...
        val weight = DnnTools.getTensor(s"Bwrd_${bn.getName}.Grad.3", Array(channel), identity)
        val bias = DnnTools.getTensor(s"Bwrd_${bn.getName}.Grad.4", Array(channel), identity)
        val weightAndBias = Tensor[Float].resize(Array(2, channel))
        weightAndBias.select(1, 1).copy(weight)
        weightAndBias.select(1, 2).copy(bias)
        ...
        val runningMean = DnnTools.getTensor(s"Fwrd_$name.Wght.0", Array(channel), identity)
        val runningVariance = DnnTools.getTensor(s"Fwrd_$name.Wght.1", Array(channel), identity)
      ...
      case _ =>
        for (j <- params.indices) {
          val w = DnnTools.getTensor(s"Bwrd_$name.Grad.$j", params(j).size(), identity)
          module match {
            case layer: MklDnnLayer =>
